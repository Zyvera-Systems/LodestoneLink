...
object TeleportMenu {
    ...
    fun open(player: Player, compassItem: ItemStack) {
        ...
        val futures = locations.map { loc ->
            NamedLodestoneManager.getLodestoneDisplay(loc).thenApply { name ->
                loc to name
            }
        }
        ...
        CompletableFuture.allOf(*futures.toTypedArray()).thenAccept {
            ...
            futures.forEachIndexed { i, future ->
                val (loc, pair) = future.join()
                if (pair != null) {
                    val itemName = pair.first
                    val displayMat = pair.second

                    val menuItem = ItemStack(
                        displayMat ?: when (loc.world.name) {
                    ...
                    val menuItem = ItemStack(Material.BARRIER)
                    val meta = menuItem.itemMeta
                    meta.displayName(mm.deserialize(ConfigUtil.config.getStringNn("messages.unsafe")))
                    menuItem.itemMeta = meta
                    inv.setItem(i, menuItem)
                }
            }
